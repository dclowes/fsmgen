...
def generate_source(the_fsm, SourceData, source_file):
    ...
    fsm_gcc = StateMachine_GCC3(fsm_sql2)
    print '\n'.join(fsm_gcc.gen_bdy_prefix())
    print '\n'.join(fsm_gcc.gen_bdy_actions())
    print '\n'.join(fsm_gcc.gen_bdy_events())
    print '\n'.join(fsm_gcc.gen_bdy_states())
#   fsm_gcc = StateMachine_GCC2(fsm_sql2)
#   hdr_gcc, txt_gcc = fsm_gcc.Generate_C()
#   hdr_gcc.insert(0, '/* Generated by program */')
#   txt_gcc.insert(0, '#include "%s2.h"' % basename)
#   txt_gcc.insert(0, '/* Generated by program */')
#   with open('%s2.h' % dest_file, 'w') as fdo:
#       fdo.write('\n'.join(hdr_gcc))
#       fdo.write('\n')
#   with open('%s2.c' % dest_file, 'w') as fdo:
#       fdo.write('\n'.join(txt_gcc))
#       fdo.write('\n')
    ...
    # Insert the GCC Header file
    text.write('<TR><TD ALIGN="right">C Header file</TD></TR>\n')
    text.write('<TR><TD ALIGN="left">\n')
    text.write('<PRE>' + escape('\n'.join(hdr_gcc)) + '</PRE>')
    text.write('</TD></TR>\n')
    # Insert the GCC Code file
    text.write('<TR><TD ALIGN="right">C Code file</TD></TR>\n')
    text.write('<TR><TD ALIGN="left">\n')
    text.write('<PRE>' + escape('\n'.join(txt_gcc)) + '</PRE>')
    text.write('</TD></TR>\n')

    # Skeleton implementation file
    fsm_gcc.Absorb_Skel('%s.skel.c' % dest_file)
    hdr_skl, txt_skl = fsm_gcc.Generate_Skel()
    if len(hdr_skl) > 0:
        hdr_skl.insert(0, '/* Generated by program */')
        with open('%s.skel.h' % dest_file, 'w') as fdo:
            fdo.write('\n'.join(hdr_skl))
            fdo.write('\n')
