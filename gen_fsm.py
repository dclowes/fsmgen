...
def generate_source(the_fsm, SourceData, source_file):
    ...
    # Generate the SQL
    #
    fsm_sql = StateMachine_SQL(the_fsm)
    fsm_sql.Generate()
    ...
    # Generate the UML
    #
    fsm_uml = StateMachine_UML(the_fsm)
    ...
    #
    fsm_html = StateMachine_HTML(the_fsm)
    ...
    # Generate the HTML
    #
    fsm_html.Generate(dest_file, SourceData, txt_text)

    #
    # Generate the TCL
    #
    if 'TCL' in the_fsm.outputs:
        from generate_tcl import StateMachine_TCL
        fsm_tcl = StateMachine_TCL(the_fsm)
        if 'SCT' in the_fsm.outputs:
            print "SCT modification"
            fsm_tcl.script_context = True
        txt_tcl = fsm_tcl.Generate_TCL()
        txt_tcl.insert(0, '# Generated by program')
        file_name = '%s.tcl' % dest_file
        with open(file_name, 'w') as fdo:
            fdo.write('\n'.join(txt_tcl))
        file_name = '%s.code.tcl' % dest_file
        fsm_tcl.Absorb_Skel(file_name)
        txt_tcl = fsm_tcl.Generate_Skel()
        txt_tcl.insert(0, '# Generated by program')
        with open(file_name, 'w') as fdo:
            fdo.write('\n'.join(txt_tcl))
    #
    ...
    if 'PYTHON' in the_fsm.outputs:
        from generate_python import StateMachine_Python
        fsm_python = StateMachine_Python(the_fsm)
    ...
    # Generate the GCC2
    #
    if 'GCC2' in the_fsm.outputs:
        from generate_gcc2 import StateMachine_GCC2
    ...
    #
    if 'GCC' in the_fsm.outputs:
        from generate_gcc3 import StateMachine_GCC3
        fsm_gcc3 = StateMachine_GCC3(StateMachine_SQL(dest_file + ".sqlite"))
